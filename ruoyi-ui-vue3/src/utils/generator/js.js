...
function buildBeforeUpload(conf) {
  ...
  if (conf.fileSize) {
    ...
    if(!isRightSize){
      proxy.$modal.msgError(translatePhrase('文件大小超过 {size}{unit}', { size: ${conf.fileSize}, unit: '${conf.sizeUnit}' }))
    }`
  ...
  if (conf.accept) {
    ...
    if(!isAccept){
      proxy.$modal.msgError(translatePhrase('应该选择 {type} 类型的文件', { type: '${conf.accept}' }))
    }`
