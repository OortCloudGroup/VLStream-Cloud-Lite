  ...
  <div>
      <el-form :model="queryParams" ref="queryRef" :inline="true" v-show="showSearch">
        <el-form-item :label="$tp('设备编码')" prop="deviceId">
          <el-input v-model="queryParams.deviceId" :placeholder="$tp('请输入客户端名称')" clearable disabled />
        </el-form-item>
        ...
        <el-table-column :label="$tp('操作')" align="center" class-name="small-padding fixed-width" fixed="right" width="100">
          <template #default="scope">
            <el-button link type="primary" icon="Delete" @click="handleDelete(scope.row)"
                       style="color: #f56c6c"
                       v-hasPermi="['viid:vehicle:remove']">{{ $tp("删除") }}
            </el-button>
          </template>
        </el-table-column>
      </el-table>
...
<script setup name="GatherNonmotorVehicleManage">
import {pageNonMotorVehicle, delNonMotorVehicles} from "@/api/gat1400/nonmotorvehicle.js";
...
const tableColumns = ref([
  { prop: 'nonMotorVehicleId', get label() { return translatePhrase("非机动车标识") }, show: true },
  ...
  { prop: 'appearTime', get label() { return translatePhrase("车辆出现时间") }, show: true },
  { prop: 'plateNo', get label() { return translatePhrase("车牌号") }, show: true },
  { prop: 'vehicleColor', get label() { return translatePhrase("车身颜色") }, show: true },
  { prop: 'vehicleType', get label() { return translatePhrase("车辆款型") }, show: false },
  { prop: 'vehicleBrand', get label() { return translatePhrase("车辆品牌") }, show: true },
  { prop: 'vehicleLength', get label() { return translatePhrase("车辆长度") }, show: true },
  { prop: 'vehicleType', get label() { return translatePhrase("车辆款型") }, show: true }
]);
...
function handleDelete(row) {
  proxy.$modal.confirm(translatePhrase("是否确认删除非机动车编号为\"") + row.id + translatePhrase("\"的数据项？")).then(function () {
...
async function getList() {
  loading.value = true;
  const res = await pageNonMotorVehicle(queryParams.value);
