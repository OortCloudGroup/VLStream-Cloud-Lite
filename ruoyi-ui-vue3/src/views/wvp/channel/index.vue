<template>
  <div class="detail-page">
    ...
    <div class="detail-body">
      ...
      >
        <el-table-column prop="name" :label="$tp('名称')" :min-width="clacPXToVW(180)" align="center" show-overflow-tooltip/>
        <el-table-column prop="deviceId" :label="$tp('编号')" :min-width="clacPXToVW(180)" align="center" show-overflow-tooltip/>
        <el-table-column :label="$tp('快照')" :min-width="clacPXToVW(100)" align="center">
          <template #default="scope">
            <ImagePreview :src="getSnap(scope.row)"></ImagePreview>
          </template>
        </el-table-column>
        <el-table-column prop="subCount" :label="$tp('子节点数')" :min-width="clacPXToVW(100)" align="center"/>
        <el-table-column prop="channelType" :label="$tp('通道类型')" :min-width="clacPXToVW(100)" align="center">
          <template #default="scope">
            <el-tag v-if="scope.row.channelType === 0">{{ $tp("国标设备") }}</el-tag>
            <el-tag v-if="scope.row.channelType === 1">{{ $tp("推流设备") }}</el-tag>
            <el-tag v-if="scope.row.channelType === 2">{{ $tp("拉流代理") }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column prop="manufacturer" :label="$tp('厂家')" :min-width="clacPXToVW(100)" align="center" show-overflow-tooltip/>
        <el-table-column :label="$tp('位置信息')" :min-width="clacPXToVW(120)" align="center">
          <template #default="scope">
            <span v-if="scope.row.longitude && scope.row.latitude">{{ scope.row.longitude }}<br/>{{ scope.row.latitude }}</span>
            <span v-else>{{ $tp("无") }}</span>
          </template>
        </el-table-column>
        <el-table-column prop="ptzType" :label="$tp('云台类型')" :min-width="clacPXToVW(100)" align="center">
          <template #default="scope">
            <div>{{ scope.row.ptzTypeText }}</div>
          </template>
        </el-table-column>
        <el-table-column :label="$tp('开启音频')" :min-width="clacPXToVW(100)" align="center">
          <template #default="scope">
            <el-switch @change="updateChannel(scope.row)" v-model="scope.row.hasAudio" active-color="#409EFF"/>
          </template>
        </el-table-column>
        <el-table-column :label="$tp('码流类型')" :min-width="clacPXToVW(180)" align="center">
          <template #default="scope">
            <div v-if="checkPermi(['wvp:device:channelStreamIdentification'])">
              <el-select
                ...
                :placeholder="$tp('请选择码流类型')"
                default-first-option
              >
                <el-option :label="$tp('stream:0(主码流)')" value="stream:0"></el-option>
                <el-option :label="$tp('stream:1(子码流)')" value="stream:1"></el-option>
                <el-option :label="$tp('streamnumber:0(主码流-2022)')" value="streamnumber:0"></el-option>
                <el-option :label="$tp('streamnumber:1(子码流-2022)')" value="streamnumber:1"></el-option>
                <el-option :label="$tp('streamprofile:0(主码流-大华)')" value="streamprofile:0"></el-option>
                <el-option :label="$tp('streamprofile:1(子码流-大华)')" value="streamprofile:1"></el-option>
                <el-option :label="$tp('streamMode:main(主码流-水星+TP-LINK)')" value="streamMode:main"></el-option>
                <el-option :label="$tp('streamMode:sub(子码流-水星+TP-LINK)')" value="streamMode:sub"></el-option>
              </el-select>
            </div>
            <div v-else>
              <el-tag v-if="scope.row.streamIdentification === 'stream:0'">{{ $tp("stream:0(主码流)") }}</el-tag>
              <el-tag v-if="scope.row.streamIdentification === 'stream:1'">{{ $tp("stream:1(子码流)") }}</el-tag>
              <el-tag v-if="scope.row.streamIdentification === 'streamnumber:0'">{{ $tp("streamnumber:0(主码流-2022)") }}</el-tag>
              <el-tag v-if="scope.row.streamIdentification === 'streamnumber:1'">{{ $tp("streamnumber:1(子码流-2022)") }}</el-tag>
              <el-tag v-if="scope.row.streamIdentification === 'streamprofile:0'">{{ $tp("streamprofile:0(主码流-大华)") }}</el-tag>
              <el-tag v-if="scope.row.streamIdentification === 'streamprofile:1'">{{ $tp("streamprofile:1(子码流-大华)") }}</el-tag>
              <el-tag v-if="scope.row.streamIdentification === 'streamMode:main'">{{ $tp("streamMode:main(主码流-水星+TP-LINK)") }}</el-tag>
              <el-tag v-if="scope.row.streamIdentification === 'streamMode:sub'">{{ $tp("streamMode:sub(子码流-水星+TP-LINK)") }}</el-tag>
            </div>
          </template>
        </el-table-column>
    ...
    <el-dialog :title="$tp('编辑通道')" v-model="open" width="65%" append-to-body>
      ...
      <template #footer>
        <div class="dialog-footer">
          ...
          <el-button v-if="form.dataType === 1" @click="resetData">{{ $tp("重置") }}</el-button>
        </div>
      </template>
    </el-dialog>
    ...
    <el-dialog :title="$tp('播放视频')" v-model="openPlay" width="65%" append-to-body @close="initData">
...
const searchData = ref([
  {
    get label() { return translatePhrase("通道类型") },
    value: 'channelType',
    type: 'select',
    option: [
      { get label() { return translatePhrase("设备") }, value: 'false' },
      { get label() { return translatePhrase("子目录") }, value: 'true' }
    ],
    default: undefined
  },
  {
  ...
  ElMessageBox.confirm(
      ...
      .then(() => {
        resetChannel(form.value.gbId).then((res) => {
          ElMessage({
            type: 'success',
            message: translatePhrase("重置成功 已保存"),
          })
        })
...
function startRecord(itemData) {
  recordApi({
    ...
    ElMessage({
      type: 'success',
      message: translatePhrase("开始录像成功"),
    })
  })
}
...
function stopRecord(itemData) {
  recordApi({
    ...
    ElMessage({
      type: 'success',
      message: translatePhrase("停止录像成功"),
    })
  })
}
