<template>
  <div class="container">
      ...
      <el-scrollbar class="left-scrollbar">
        <div class="components-list">
          <div class="components-title">
            <svg-icon icon-class="component" />{{ $tp("输入型组件") }}
          </div>
          ...
          <div class="components-title">
            <svg-icon icon-class="component" />{{ $tp("选择型组件") }}
          </div>
          ...
          <div class="components-title">
            <svg-icon icon-class="component" /> {{ $tp("布局型组件") }}
          </div>
    ...
    <code-type-dialog v-model="dialogVisible" :title="$tp('选择生成类型')" :showFileName="showFileName" @confirm="generate" />
...
function empty() {
  proxy.$modal.confirm(translatePhrase("确定要清空所有组件吗？"), translatePhrase("提示"), { type: 'warning' }).then(() => {
      idGlobal.value = 100
      drawingList.value = []
    }
  )
...
onMounted(() => {
  const clipboard = new ClipboardJS('#copyNode', {
    text: trigger => {
      const codeStr = generateCode()
      ElNotification({ title: translatePhrase("成功"), message: translatePhrase("代码已复制到剪切板，可粘贴。"), type: 'success' })
      return codeStr
    }
  })
  clipboard.on('error', e => {
    proxy.$modal.msgError(translatePhrase("代码复制失败"))
  })
})
</script>
