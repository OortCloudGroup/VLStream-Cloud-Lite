  ...
  <div class="app-container">
    <div class="toolbar-with-search">
      ...
      <div class="searchHeight_out flexRowAC">
        <search-height-box
          keyword="name"
          :placeholder="$tp('请输入设备名称、设备ID、IP等关键词')"
          :data="searchData"
    ...
    >
      ...
      <el-table-column :label="$tp('设备ID')" align="center" prop="deviceId" show-overflow-tooltip/>
      ...
      <el-table-column :label="$tp('IP地址')" align="center" prop="ipAddress" show-overflow-tooltip/>
      ...
      <el-table-column :label="$tp('状态')" align="center" prop="status" :width="clacPXToVW(90)">
        <template #default="scope">
        ...
        <template #default="scope">
          <div class="operateAppBox flexRowAC" style="justify-content: flex-end;">
            ...
            >
              ...
              <template #dropdown>
                <el-dropdown-menu>
                  <el-dropdown-item command="handleUpdate" v-if="checkPermi(['isup:lsupDevice:edit'])">{{ $tp("修改") }}</el-dropdown-item>
                  <el-dropdown-item command="handleMap" v-if="checkPermi(['isup:lsupDevice:edit'])">{{ $tp("修改位置") }}</el-dropdown-item>
                  <el-dropdown-item command="handleDelete" style="color: #f56c6c" v-if="checkPermi(['isup:lsupDevice:remove'])">{{ $tp("删除") }}</el-dropdown-item>
                </el-dropdown-menu>
              </template>
            </el-dropdown>
          </div>
    ...
    <el-dialog :title="$tp('播放视频')" v-model="openPlay" width="55%" append-to-body>
      <div>
    ...
    <el-dialog :title="$tp('SDK播放')" v-model="openPlaySDK" width="55%" append-to-body @close="closeSDK">
      <div>
...
const searchData = ref([
  ...
  { get label() { return translatePhrase("设备ID") }, value: 'deviceId', type: 'text', default: undefined },
  { get label() { return translatePhrase("IP地址") }, value: 'ipAddress', type: 'text', default: undefined },
  {
    get label() { return translatePhrase("状态") },
    value: 'status',
    type: 'select',
    option: [
      { get label() { return translatePhrase("在线") }, value: 'ON' },
...
const toolbarButtons = computed(() => [
  { get name() { return translatePhrase("修改") }, svg: 'edit', disabled: single.value, permi: ['isup:lsupDevice:edit'], clickFn: () => handleUpdate() },
  { get name() { return translatePhrase("删除") }, svg: 'delete', disabled: multiple.value, permi: ['isup:lsupDevice:remove'], clickFn: () => handleDelete() }
]);
...
const data = reactive({
  ...
  rules: {
    ...
    name: [
      {required: true, get message() { return translatePhrase("请输入设备名称") }, trigger: "blur"}
    ],
    channel: [
      {required: true, get message() { return translatePhrase("请选择通道") }, trigger: "change"}
    ],
...
function closeSDK() {
  stopRealPlay(luserId.value).then(() => {
    proxy.$modal.msgSuccess(translatePhrase("关闭播放"));
  })
}
...
function handleDelete(row) {
  ...
  proxy.$modal.confirm(translatePhrase("是否确认删除isup设备编号为\"") + _ids + translatePhrase("\"的数据项？")).then(function () {
