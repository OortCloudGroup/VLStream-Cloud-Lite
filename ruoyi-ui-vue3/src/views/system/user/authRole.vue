<template>
   <div class="detail-page">
      <detail-page-header
         :parent-title="$tp('用户管理')"
         :title="$tp('分配角色')"
         back-path="/system/user"
      />
      <div class="detail-body">
      <h4 class="form-header h4">{{ $tp("基本信息") }}</h4>
      <el-form :model="form" label-width="80px">
         <el-row>
            ...
            <el-col :span="8" :offset="2">
               <el-form-item :label="$tp('登录账号')" prop="userName">
      ...
      <h4 class="form-header h4">{{ $tp("角色信息") }}</h4>
      <table-self v-loading="loading" :row-key="getRowKey" @row-click="clickRow" ref="roleRef" @selection-change="handleSelectionChange" :data="roles.slice((pageNum - 1) * pageSize, pageNum * pageSize)" class="new_table" header-cell-class-name="header_tenant_cell" stripe>
         ...
         <el-table-column :label="$tp('角色编号')" align="center" prop="roleId" />
         <el-table-column :label="$tp('角色名称')" align="center" prop="roleName" />
         <el-table-column :label="$tp('权限字符')" align="center" prop="roleKey" />
...
function submitForm() {
  ...
  updateAuthRole({ userId: userId, roleIds: rIds }).then(response => {
    proxy.$modal.msgSuccess(translatePhrase("授权成功"));
    close();
  });
};
