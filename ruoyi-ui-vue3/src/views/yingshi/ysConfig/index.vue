...
import {addYsConfig, delYsConfig, getYsConfig, listYsConfig, updateYsConfig} from "@/api/yingshi/ysConfig";

const { proxy } = getCurrentInstance();
const { yingshi_enable } = proxy.useDict('yingshi_enable');
...
function getList() {
  ...
  listYsConfig(queryParams.value).then(response => {
    ysConfigList.value = response.rows;
...
function handleUpdate(row) {
  ...
  getYsConfig(_id).then(response => {
    form.value = response.data;
    open.value = true;
    title.value = "修改萤石监控配置";
  });
}
...
function handleExport() {
  proxy.download('yingshi/ysConfig/export', {
    ...queryParams.value
  }, `ysConfig_${new Date().getTime()}.xlsx`)
}
