  ...
  <el-dialog :title="$tp(dialog.title)" v-model="dialog.visible" width="32%" append-to-body>
    <el-form ref="equipmentSupportFormRef" :model="form" :rules="rules" label-width="100px">
      <el-form-item :label="$tp('设备型号')" prop="model">
        <el-input v-model="form.model" show-word-limit :maxlength="50" :placeholder="$tp('请输入设备型号')"/>
      </el-form-item>
      <el-form-item :label="$tp('设备版本号')" prop="version">
        <el-input v-model="form.version" show-word-limit :maxlength="50" :placeholder="$tp('请输入设备版本号')"/>
      </el-form-item>
    </el-form>
...
const rules = ref({
  model: [
    {required: true, get message() { return translatePhrase("请输入设备型号") }, trigger: "blur"}
  ],
  version: [
    {required: true, get message() { return translatePhrase("请输入设备版本号") }, trigger: "blur"}
  ]
});
...
const show = async (configId) => {
  dialog.visible = true
  dialog.title = translatePhrase("设备支持萤石协议")
  ...
  equipmentSupportFormRef.value?.validate(async (valid) => {
    if (valid) {
      ...
      if (res.data.isSupport === 1) {
        proxy?.$modal.msgSuccess(res.data.model + translatePhrase("设备支持"));
      } else {
        proxy?.$modal.msgError(res.data.model + translatePhrase("设备不支持"));
      }
